...
export function GeneralChat({ lifeStage }: { lifeStage: string }) {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    setMessages(prev => [...prev, { role: "user", content: userMsg, reflection: "", reassurance: "", suggestion: "" }]);
    setIsLoading(true);
    ...
    try {
      ...
      const data = await getAiResponse({
        ...
        isPractice: false,
        imageBase64: null // You can hook this up to your Paperclip button later
      });
      ...
      setMessages(prev => [...prev, { 
        ...
        reflection: "", reassurance: "", suggestion: "" 
      }]);
    } finally {
      setIsLoading(false);
    }
            ...
            >
              <div className={`flex gap-4 max-w-[85%] ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
                <div className={`w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 shadow-sm border ${
                  msg.role === "user" ? "bg-slate-50 text-slate-400" : "bg-indigo-600 text-white"
                }`}>
                  {msg.role === "user" ? <User size={20} /> : <Bot size={20} />}
                </div>
                
                <div className={`flex flex-col gap-2 ${msg.role === "user" ? "items-end" : "items-start"}`}>
                  <div className={`p-5 rounded-[2.2rem] text-[16px] font-medium leading-relaxed border shadow-sm ${
                    msg.role === "user" 
                  ...
                  {msg.role === "ai" && msg.suggestion && (
                    ...
                    >
                      <strong className="block mb-1 text-indigo-600 uppercase text-[10px] tracking-widest">Mirror Suggestion:</strong>
                      {msg.suggestion}
                    </motion.div>
                  )}
                </div>
          ...
          {isLoading && (
            <div className="flex justify-start gap-4 items-center ml-14">
               <div className="bg-white p-3 rounded-full border border-slate-100 flex items-center gap-2 shadow-sm">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Mirror is reflecting...</span>
               </div>
            </div>
          )}
        </AnimatePresence>
      ...
      <div className="p-6 bg-slate-50 border-t border-slate-100">
        <form onSubmit={handleSendMessage} className="flex gap-4 max-w-4xl mx-auto items-center">
          <div className="relative flex-1">
            ...
            />
            <button type="button" className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-indigo-600 transition-colors">
              <Paperclip size={22} />
            </button>
          </div>
          
          <button type="button" className="h-16 w-16 rounded-full bg-white border border-slate-200 flex items-center justify-center text-slate-400 hover:text-indigo-600 transition-all shadow-sm">
            <Mic size={22} />
          </button>
